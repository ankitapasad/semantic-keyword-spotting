...
print("log: %s, model dir: %s" % (ddir.vs_log_fn, ddir.vs_output))
...
def train(epoch, dir_fn, label_dict, in_dir):
    ...
    for i in range(0, len(perm), args.batch_size):
        ...
        l, pred, grt, _ = run_net(label_dict, in_dir, fns)
        ...
        if len(lt) % args.print_interval == 0:
            ...
            print(pcont)
            with open(ddir.vs_log_fn, "a+") as fo:
                fo.write(pcont+"\n")
            lt, pred_multi, grt_multi = [], [], []
    return 0
...
def apply_train():
    label_dict = load_label_dict(ddir.caption_fn)
    best_fscore = -1
    for epoch in range(args.epoch):
        train(epoch, ddir.train_dir_fn, label_dict, ddir.map_dir)
        fscore = test(epoch, ddir.dev_dir_fn, label_dict, ddir.map_dir)
        if fscore > best_fscore:
            torch.save(mlp, ddir.vs_model_fn)
            best_fscore = fscore
    return 0
...
def apply_test():
    ...
    for i in range(0, len(fc_fns), args.test_batch_size):
        ...
        # fcs = load_fc(fc_dir, fns_i)
        fcs = load_map(fc_dir, fns_i)
        fcs = torch.from_numpy(fcs).to(args.device)
        probs, max_idx = mlp(fcs)
        probs, max_idx = probs.cpu().data.numpy(), max_idx.cpu().data.numpy()
